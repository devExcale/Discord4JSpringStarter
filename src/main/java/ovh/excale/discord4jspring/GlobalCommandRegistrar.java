...
public class GlobalCommandRegistrar implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) throws IOException {
		...
		final long applicationId = client
				.getApplicationId()
				.block();
		...
		for(Resource resource : matcher.getResources("commands/*.json")) {
			...
			if(!discordCommands.containsKey(request.name())) {
				...
				log.info("Created global command: " + request.name());
			}
		}
	...
	private boolean hasChanged(ApplicationCommandData discordCommand, ApplicationCommandRequest command) {
		...
		boolean discordCommandDefaultPermission = discordCommand
				.defaultPermission()
				.toOptional()
				.orElse(true);
		boolean commandDefaultPermission = command
				.defaultPermission()
				.toOptional()
				.orElse(true);
